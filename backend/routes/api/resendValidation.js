...
resendValidation.route('/').post((req, res) => {
    ...
    User.findOne({email: userEmail }, function (err, user) {
        if (!user) {
            return res.status(400).send({ type: 'no-account', msg: 'This user does not yet exist, please make an account with the signup feature' });
        } 
        ...
        else
        {
            ...
            res.status(200).send('A verification email has been sent to ' + userEmail + ' with the resent Token.');
